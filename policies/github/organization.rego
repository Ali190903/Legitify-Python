...
organization_webhook_no_secret[violated] := true if {
	some index
...
organization_webhook_doesnt_require_ssl[violated] := true if {
	some index
	hook := input.hooks[index]
...
two_factor_authentication_not_required_for_org := false if {
...
non_admins_can_create_public_repositories := false if {
...
default_repository_permission_is_not_none := false if {
...
organization_not_using_single_sign_on := false if {
	input.saml_enabled
}
