...
repository_not_maintained := false if {
...
repository_has_too_many_admins := false if {
...
repository_webhook_no_secret[violated] := true if {
	some index
...
repository_webhook_doesnt_require_ssl[violated] := true if {
	some index
	hook := input.hooks[index]
...
forking_allowed_for_repository := false if {
...
missing_default_branch_protection := false if {
...
missing_default_branch_protection_deletion := false if {
...
missing_default_branch_protection_force_push := false if {
...
requires_status_checks := false if {
...
requires_branches_up_to_date_before_merge := false if {
	input.repository.default_branch.branch_protection_rule.requires_status_checks
...
dismisses_stale_reviews := false if {
...
code_review_not_required := false if {
...
code_review_by_two_members_not_required := false if {
...
code_review_not_limited_to_code_owners := false if {
...
no_conversation_resolution := false if {
...
review_dismissal_allowed := false if {
...
pushes_are_not_restricted := false if {
...
vulnerability_alerts_not_enabled := false if {
...
ghas_dependency_review_not_enabled := false if {
...
scorecard_score_too_low := false if {
...
token_default_permissions_is_read_write := false if {
...
actions_can_approve_pull_requests := false if {
...
users_allowed_to_bypass_ruleset := false if {
...
repository_secret_is_stale[stale] := true if {
    some index
...
secret_scanning_not_enabled := false if {
